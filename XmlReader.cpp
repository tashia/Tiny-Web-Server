...
XmlReader::attribElems XmlReader::attributes() {
	...
	while (true) {
		...
		size_t quotePos1 = localposition;
		quotePos1 = _xml.find_first_of("\"\'", quotePos1);
		// find second quote position
		localposition = _xml.find_first_of("\"\'", quotePos1 + 1);
		std::string value = _xml.substr(quotePos1 + 1, localposition
				- quotePos1 - 1);
